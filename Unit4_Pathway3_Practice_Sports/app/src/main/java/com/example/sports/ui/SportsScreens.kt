...
// Window Size Class (cho adaptive layout)
...
fun SportsApp(
    ...
    val sportsContentType: SportsContentType
    val viewModel: SportsViewModel = viewModel()
    val uiState by viewModel.uiState.collectAsState()

    when (windowSize) {
        WindowWidthSizeClass.Compact -> {
            sportsContentType = SportsContentType.ListOnly
        }
        ...
        WindowWidthSizeClass.Expanded -> {
            sportsContentType = SportsContentType.ListAndDetail
        }
    ...
    if (sportsContentType == SportsContentType.ListOnly) {
        SportsAppListOnly(
            ...
            navigateToListPage = { viewModel.navigateToListPage() },
            updateCurrentSport = { sport -> viewModel.updateCurrentSport(sport) },
            navigateToDetailPage = { viewModel.navigateToDetailPage() },
            sportsList = uiState.sportsList,
        )
    } else {
        SportsAppListAndDetail(
            currentSport = uiState.currentSport,
            updateCurrentSport = { sport -> viewModel.updateCurrentSport(sport) },
            sportsList = uiState.sportsList,
        )
...
@Composable
fun SportsAppListAndDetail(
    currentSport: Sport,
    updateCurrentSport: (Sport) -> Unit,
    ...
    Scaffold(
        topBar = {
            SportsAppBar(
                ...
                onBackButtonClick = { navigateToListPage() },
            )
        }
    ) { innerPadding ->
